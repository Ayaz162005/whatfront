...
import { useCreateUser } from "../graphqlActions/CreateUser";
import { useGetAllUsers } from "../graphqlActions/GetAllUsers";
...
const useStyles = makeStyles((theme) => ({
  ...
  imageContainer: {
    ...
    backgroundImage: "url('/public/images/signupback.avif')", // Replace with your image path
...
const SignupPage = () => {
  ...
  useEffect(() => {
    // You can access data, error, loading here
  }, [data]);
  ...
  const handleSubmit = async (event) => {
    try {
      ...
      // Call createUser and await its response
      ...
      localStorage.setItem("token", res.data.createUser.token.access_token);
  ...
  const validateForm = () => {
    ...
    if (formData.username.trim().length < 6) {
      isValid = false;
      newFormErrors.username = "Username must be at least 6 characters long";
    }
    // Validate email
        ...
        <div className={classes.formContainer}>
          ...
          <form className={classes.form} onSubmit={handleSubmit}>
            ...
            <Button
              ...
              variant="contained"
              fullWidth
              className={`${classes.submitButton} ${classes.googleButton}`}
            >
              Sign Up with Google
            </Button>
